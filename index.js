...
async function askAssistant(messages=[]) {
  ...
  for (const m of messages) await postMessage(thread_id, m.role, m.content);
  const { run_id } = await runAssistant(thread_id);
  await pollUntilDone(thread_id, run_id);
  return await fetchReply(thread_id);
}
